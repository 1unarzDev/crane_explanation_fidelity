...
TWENTY_THIRD_ATTEMPT = ROOT / "manifests/data/cm-land-conf-059-disposition.json"
TWENTY_FOURTH_ATTEMPT = ROOT / "manifests/data/cm-land-conf-060-disposition.json"
...
def test_second_later_window_recovery_run_is_primary_and_causally_bounded():
    run = load(TWENTY_THIRD_ATTEMPT)
    reference = run["method_visible_reference"]
    ...
    assert run["semantic_boundary"]["P_responses"] == 0
    assert run["semantic_boundary"]["R_responses"] == 0
    assert run["focused_progress"]["primary_diagnosable_references"] == 16
    assert run["focused_progress"]["next_fixed_run_id"] == "cm-land-conf-060"


def test_later_window_connected_detour_persistent_run_retains_abort_and_limits():
    run = load(TWENTY_FOURTH_ATTEMPT)
    reference = run["method_visible_reference"]
    ...
    assert run["observed"]["navigation_status"] == "aborted"
    assert reference["primary_endpoint_eligible"] is True
    assert reference["discrepancy_interval_s"] == [11.0, 21.0]
    assert reference["discrepancy_commanded_planar_speed_mps"] == 0.26
    assert reference["discrepancy_measured_planar_speed_mps"] == 0.0
    assert reference["response_recovery_interval_s"] is None
    assert reference["unique_physical_cause_supported"] is False
    assert run["semantic_boundary"]["P_responses"] == 0
    assert run["semantic_boundary"]["R_responses"] == 0
    assert run["focused_progress"]["primary_diagnosable_references"] == 17
    assert run["focused_progress"]["next_fixed_run_id"] == "cm-land-conf-061"


def test_third_later_window_recovery_run_is_primary_and_causally_bounded():
    run = load(TWENTY_FIFTH_ATTEMPT)
    reference = run["method_visible_reference"]
    ...
    assert run["semantic_boundary"]["P_responses"] == 0
    assert run["semantic_boundary"]["R_responses"] == 0
    assert run["focused_progress"]["primary_diagnosable_references"] == 18
    assert run["focused_progress"]["next_fixed_run_id"] == "cm-land-conf-062"


def test_later_window_nominal_route_persistent_run_retains_abort_and_limits():
    run = load(TWENTY_SIXTH_ATTEMPT)
    reference = run["method_visible_reference"]
    ...
    assert run["observed"]["navigation_status"] == "aborted"
    assert reference["primary_endpoint_eligible"] is True
    assert reference["discrepancy_interval_s"] == [11.0, 22.0]
    assert reference["response_ratio"] == 0.0
    assert reference["response_recovery_interval_s"] is None
    assert reference["unique_physical_cause_supported"] is False
    assert run["semantic_boundary"]["P_responses"] == 0
    assert run["semantic_boundary"]["R_responses"] == 0
    assert run["focused_progress"]["primary_diagnosable_references"] == 19
    assert run["focused_progress"]["next_fixed_run_id"] == "cm-land-conf-063"


def test_connected_detour_recovery_run_is_primary_and_causally_bounded():
    run = load(TWENTY_SEVENTH_ATTEMPT)
    reference = run["method_visible_reference"]
    ...
    assert reference["discrepancy_interval_s"] == [18.0, 28.0]
    assert reference["response_recovery_interval_s"] == [30.0, 31.0]
    assert reference["recovered_response_ratio"] == 0.9615384877957853
    assert reference["recovery_caused_outcome_supported"] is False
    assert reference["unique_physical_cause_supported"] is False
    assert run["semantic_boundary"]["P_responses"] == 0
    assert run["semantic_boundary"]["R_responses"] == 0
...
def test_connected_detour_late_persistent_run_is_primary_and_causally_bounded():
    run = load(TWENTY_NINTH_ATTEMPT)
    reference = run["method_visible_reference"]
    ...
    assert run["observed"]["navigation_status"] == "aborted"
    assert reference["primary_endpoint_eligible"] is True
    assert reference["discrepancy_interval_s"] == [19.0, 29.0]
    assert reference["discrepancy_commanded_planar_speed_mps"] == 0.25
    assert reference["response_recovery_interval_s"] is None
    assert reference["unique_physical_cause_supported"] is False
    assert run["semantic_boundary"]["P_responses"] == 0
    assert run["semantic_boundary"]["R_responses"] == 0
    assert run["focused_progress"]["primary_diagnosable_references"] == 22
    assert run["focused_progress"]["next_fixed_run_id"] == "cm-land-conf-066"


def test_shorter_connected_detour_recovery_run_is_primary_and_causally_bounded():
    run = load(THIRTIETH_ATTEMPT)
    reference = run["method_visible_reference"]
    ...
    assert reference["primary_endpoint_eligible"] is True
    assert reference["discrepancy_interval_s"] == [10.0, 20.0]
    assert reference["response_recovery_interval_s"] == [23.0, 24.0]
    assert reference["recovered_response_ratio"] == 1.0000000050575335
    assert reference["recovery_caused_outcome_supported"] is False
    assert reference["unique_physical_cause_supported"] is False
    assert run["semantic_boundary"]["P_responses"] == 0
    assert run["semantic_boundary"]["R_responses"] == 0
