...
| Independently derived physical/execution diagnoses add value beyond causal statements already present in logs | Current development computations and separately implemented references; no confirmatory semantic comparison | Fresh independent clusters, fair tool-enabled R, frozen semantic endpoint, valid uncertainty bounds | Prospective diagnosis-versus-repetition cases | Provisional contribution only; claim improvement only if prospective results support it |
