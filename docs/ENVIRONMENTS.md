...
| Configurable land proving ground | Manifest-driven eight-motif generator with separate canonical/presentation layers, seeded configuration hashes, semantic IDs, evaluator truth, dynamic schedules, dedicated Nav2 parameters, inspection overlays, and hash-checking behavioral/repetition QA; additive v3 supplies a bounded temporary enclosure without changing v1/v2 | Structural/physics/sensor/headless/explanation checks pass; all eight original motifs have one behaviorally qualified development revision across preserved v1 plus additive v2 slalom; corrected S-turn, historical dynamic recovery-success, and bounded blockage each pass three exact-condition runs. The first v3 calibration succeeded with feedback recovery 1, while the fresh source-qualified capture aborted after eight observed recovery invocations; v3 is retained as an unstable calibration. The straight-through v1 slalom remains a negative calibration | Exact-hash ecological questions cover dynamic recovery-success, bounded terminal policy, S-turn path shape, U-trap route change, false premises, and causal/optimality withholding. Current-build bounded-blockage and corrected S-turn runs pass ordered capture, mechanism/route admission, and separated export. The corrected v3 run validates recovery-invocation capture but fails its required success outcome, so it remains unexported. Explanation generation/annotation remains `NOT_RUN` | **REQUIRED — terminal-policy and path-shape evidence handoffs pass; use already qualified artifacts for the explanation pilot rather than treating v3 as stable** |
