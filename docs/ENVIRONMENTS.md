...
| TurtleBot3 warehouse/industrial | Manifest-driven 20 × 26 m canonical geometry, separate presentation, semantic/route/scenario IDs, deterministic static/delayed/temporary obstacles and a temporary four-wall enclosure, differential robot and LiDAR | Structural/physics/sensor/headless gates; 13.61 m nominal detour; exact bounded-policy pair produced 53.38 s success versus 71.10 s BT-deadline abort under complete blockage; temporary-enclosure calibration succeeded after delivered recovery transitions; isolated inspection passed | Task-policy provenance, recovery mechanisms, scenario identities, evaluator-only wall timing, and delivered trajectory are QA-ready; explanation evaluation and repeated-run qualification remain `NOT_RUN`, and no physical-cause claim is made | **REQUIRED — active ecological development; nominal, terminal-policy, and one recovery-success calibration pass** |
| Configurable land proving ground | Manifest-driven eight-layout generator with separate canonical/presentation layers, seeded configuration hashes, semantic IDs, evaluator truth, dynamic schedules, dedicated Nav2 parameters, inspection overlays, and hash-checking behavioral QA | Structural/physics/sensor/headless/explanation checks pass; seven layouts have one behaviorally qualified development run; the slalom action succeeded but failed its S-turn gate and is retained as a negative geometry calibration; isolated overview/oblique inspection passed; repetition remains `NOT_RUN` | Stable layout/obstacle IDs, schedule truth, delivered BT/recovery/trajectory evidence, artifact hashes, and explicit route-shape gates are ready for ecological episode design; explanation generation/annotation remains `NOT_RUN` | **REQUIRED — active ecological development; seven layouts calibrated, slalom correction open** |
