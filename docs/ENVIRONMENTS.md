...
| Configurable land proving ground | Manifest-driven eight-layout generator with separate canonical/presentation layers, seeded configuration hashes, semantic IDs, evaluator truth, dynamic schedules, dedicated Nav2 parameters, and inspection overlays | Structural/physics/sensor/headless/explanation checks pass; alternate-corridor success, dynamic-gate recovery-success, and bounded complete-blockage abort each have one development run; isolated overview/oblique inspection passed; five layouts and repeated-run qualification remain `NOT_RUN` | Stable layout/obstacle IDs, schedule truth, delivered BT/recovery/trajectory evidence, and qualification boundaries are ready for ecological episode design; explanation generation/annotation remains `NOT_RUN` | **REQUIRED — active ecological development; core generator implemented, three layouts calibrated** |
