...
unopened. Prospective physical configurations collected are 29/64 after 30 fixed-order attempts.
...
success. Run 048 adds a second prospectively masked missing-odometry control: its complete physical
...
are missing/ambiguous controls. Source-order run 054 adds a valid nominal false-premise control:
the action succeeded with healthy response, the discrepancy diagnostic did not trigger, and no
...
retained controls;
source-order run 057 adds a later-window transient discrepancy, measured response recovery, and
success. Thus fourteen valid configurations are primary-diagnosable and six are retained controls;
source-order run 058 adds a later-window persistent discrepancy followed by a validly retained
abort. Thus fifteen valid configurations are primary-diagnosable and six are retained controls;
source-order run 059 adds another later-window transient discrepancy followed by measured response
recovery and success. Thus sixteen valid configurations are primary-diagnosable and six are
retained controls;
source-order run 060 adds a later-window persistent discrepancy followed by a validly retained
abort. Thus seventeen valid configurations are primary-diagnosable and six are retained controls;
source-order run 061 adds a later-window measured-response recovery followed by success. Thus
eighteen valid configurations are primary-diagnosable and six are retained controls;
source-order run 062 adds a later-window persistent discrepancy followed by a validly retained
abort. Thus nineteen valid configurations are primary-diagnosable and six are retained controls;
source-order run 063 adds a connected-detour discrepancy followed by measured response recovery
and success. Thus twenty valid configurations are primary-diagnosable and six are retained controls;
source-order run 064 adds a nominal-route persistent discrepancy followed by a validly retained
abort. Thus twenty-one valid configurations are primary-diagnosable and six are retained controls;
source-order run 065 adds a connected-detour persistent discrepancy followed by a validly retained
abort. Thus twenty-two valid configurations are primary-diagnosable and six are retained controls;
source-order run 066 adds a connected-detour discrepancy followed by measured response recovery
and success. Thus twenty-three valid configurations are primary-diagnosable and six are retained controls;
...
configuration is source-order `cm-land-conf-067`.
...
whose independent computation returns `not_triggered`. No language response or semantic label
exists. Run 006 adds a second independently configured missing-odometry case while retaining its
unexpected abort and execution sequence. Run 007 adds a fresh-layout successful nominal
`not_triggered` control. Run 008 adds a successful transient-compensation case with independently
reproduced response loss and recovery. Run 009 adds the first persistent-discrepancy case:
independently reproduced response loss from 10--20 s followed by action abort, while the unique
physical cause remains withheld. Run 010 adds a second persistent-discrepancy case on
nominal-clear-route geometry: response loss from 18--39 s followed by action abort. Run 011 adds
another successful transient-compensation case: response loss from 10--20 s, recovered
response from 22--23 s, and eventual action success. Run 012 adds a successful nominal
`not_triggered` control despite retained FollowPath failures and Wait invocations. Run 013 adds a
...
with one recorded failure/recovery cycle.
Run 017 adds a persistent discrepancy from 18--28 s followed by action abort. Run 018 retains the
abort and execution sequence while the declared odometry mask makes the mechanism insufficient.
Run 019 adds a connected-detour persistent discrepancy from 11--21 s followed by action abort.
Run 020 adds a nominal-clear-route persistent discrepancy from 18--28 s followed by action abort.
Its first derived export accidentally used the embedded 0.4 m/s threshold; those invalid derived
artifacts are retained evaluator-only, and the canonical artifacts were recomputed once from the
unchanged raw capture with the frozen low-speed configuration. The episode was not rerun. The next
run, 021, adds a connected-detour persistent discrepancy from 18--28 s followed by action abort.
Run 022 adds the corresponding nominal-clear-route persistent discrepancy from 18--28 s followed
by action abort. Run 023 adds a connected-detour persistent discrepancy over the same interval,
again followed by action abort. Run 024 adds a nominal-clear-route discrepancy from 10--20 s,
measured-response recovery from 22--23 s, and eventual action success. The next immutable schedule
item, run 025, adds a connected-detour discrepancy from 11--21 s followed by action abort. The
next immutable schedule item, run 026, adds a connected-detour discrepancy from 18--28 s,
measured-response recovery from 30--31 s, and eventual action success. Run 027 adds a
connected-detour persistent discrepancy from 11--21 s followed by action abort. The next immutable
schedule item, run 028, is an independently configured missing-odometry case whose method-visible
reference is `insufficient` while retaining the abort sequence. Run 029 adds the same independently
checked withholding behavior on a different frozen configuration. Run 030 adds a connected-detour
persistent discrepancy from 18--39 s followed by action abort. Run 031 adds a nominal-clear-route
...
run 035, which passed every admission and completeness gate. Proposed and independent computations
agree on a connected-detour discrepancy from 11--21 s at 0.260 versus 0.000 m/s, measured-response
recovery from 23--24 s to 0.2298 m/s, and eventual action success after 92.260 s and 17.505 m. It
adds one valid transient-compensation cluster but no response, semantic label, effect estimate, or
alpha use. Run 036 then passed every gate, succeeded after 69.010 s and 17.477 m, and independently returned
`not_triggered` with one FollowPath attempt and no recorded failure or Wait invocation. It adds one
valid nominal control and no semantic measurement or alpha. Run 037 then passed every gate;
...
success after 80.160 s and 17.467 m. Run 039 then passed every gate and adds another
nominal-clear-route transient-compensation success: discrepancy from 18--28 s, measured-response
recovery from 30--31 s, action success after 80.960 s and 17.467 m, and proposed/independent parity.
Run 040 passed the frozen build, binding, worker-recording, and independent-reference gates and is
