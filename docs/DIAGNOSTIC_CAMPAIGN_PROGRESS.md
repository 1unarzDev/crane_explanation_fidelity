...
The separate physical/reference cohort has attempted 12/100 fixed confirmation-side configurations:
12 valid, 0 invalid. The method-visible diagnostic dispositions are six supported, four
...
`not_triggered` control despite retained FollowPath failures and Wait invocations. The next
immutable schedule item is `cm-land-conf-013`.
