...
| Warehouse/industrial ecological world | **PASS:** 20 nominal canonical colliders, 20 separated renderers, 28 unique nominal semantic IDs; four manifest scenario contracts | **PASS:** drop/contact plus 0.150 m differential drive and 19.1° turn response; scenario collider/visual state changes share fixed-time boundaries | **PASS:** ROS/LiDAR/odometry, semantic hit on `rack-center-blocker`, populated costmaps | **PASS (nominal route):** 13.61 m sampled west-aisle detour, 2.00 m lateral excursion, success in 54.63 s. **NEGATIVE CALIBRATION (blockage):** stock continuous replanning stayed active until client cancellation; not an abort/recovery pass | **PARTIAL:** isolated 1280×720 overview/oblique, HUD, semantic highlight, trajectory, and collider wireframes visually passed; manual keyboard polling not directly exercised | **PASS:** matching manifest v2.1.0 geometry and nominal route under null graphics | **PASS for identity/truth contract:** stable environment/route/scenario/object IDs and evaluator-only timing; physical consumption and terminal-mechanism claims remain bounded | **PARTIAL overall:** explicit bounded ecological BT, recovery/terminal-failure qualification, and full interactive check remain |
