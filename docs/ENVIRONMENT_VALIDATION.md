...
| Configurable proving ground | **PASS:** eight-layout v1 manifest; separate canonical/presentation roots; per-layout validators report 5–6 canonical colliders, matching collider-free renderers, 7–8 unique semantic IDs, zero duplicates | **PASS for qualified layouts:** collision/drop support, 0.150 m differential motion and 19.1° turn response; dynamic gate uses recorded fixed-simulation-time activation/removal; narrow doorway preserves its declared 1.0 m opening | **PASS:** ROS/LiDAR/odometry path, populated costmaps, and semantic hits on layout-relevant obstacles | **PARTIAL:** alternate-corridor and narrow-doorway successes; U-trap success after 14 sampled reverse intervals and 3.003 m exterior excursion; dynamic-gate recovery-success; bounded complete-blockage abort. Each observed once; three layouts and repeated qualification `NOT_RUN` | **PARTIAL:** isolated overview/oblique HUD, semantic highlight, trajectory, and collider overlays passed; manual keyboard polling `NOT_RUN` | **PASS:** matching v1 manifest geometry plus five route runs under null graphics | **PASS for development contracts:** stable layout/obstacle IDs, exact manifest/configuration and artifact hashes, evaluator-only schedule truth, delivered BT/recovery/trajectory evidence; physical cause and controller consumption are not inferred | **PARTIAL overall:** generator and five priority calibrations pass; three layouts, repetition, and explanation evaluation remain |
