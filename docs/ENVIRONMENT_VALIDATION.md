...
| RoboBoat focused diagnostic domain | **PASS:** integrated scene, vehicle, dock, and fixture identities | **PASS:** calibrated command axes/response and retained default aquatic physics | **PASS:** ROS/TF/odometry, command, costmap, water-observation, and independent docking-evaluator evidence in retained runs | **PASS for development baseline:** known-path docking and five repeated far-dock Nav2 actions/physical predicates succeeded; 3/5 were strict generic-valid because two had depth-camera startup loss | Passed in retained visual/runtime validation | Aquatic runtime requires graphics; no headless claim | **PARTIAL:** terminal-margin R/P/T/N outputs and packets plus retained-grid diagnostic exist; no prospective surface explanation cohort or human labels | **INTEGRATED / PROTECTED — development-ready diagnostic source, prospective evaluation `NOT_RUN`** |
