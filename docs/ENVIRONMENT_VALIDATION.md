...
| Configurable proving ground | **PASS:** eight-layout v1 manifest; separate canonical/presentation roots; per-layout validators report 5–7 canonical colliders, matching collider-free renderers, 7–9 unique semantic IDs, zero duplicates | **PASS for qualified layouts:** collision/drop support, 0.150 m differential motion and 19.1° turn response; dynamic gate uses recorded fixed-simulation-time activation/removal; narrow doorway preserves its declared 1.0 m opening | **PASS:** ROS/LiDAR/odometry path, populated costmaps, and semantic hits on layout-relevant obstacles | **PARTIAL:** seven layouts pass separately versioned trajectory/recovery gates once. Offset gates traversed +1.597/-1.455 m laterally; staggered obstacles changed lateral direction three times; the slalom action succeeded but remained exactly on centerline and therefore fails its S-turn gate. Repetition is `NOT_RUN` | **PARTIAL:** isolated overview/oblique HUD, semantic highlight, trajectory, and collider overlays passed; manual keyboard polling `NOT_RUN` | **PARTIAL overall:** matching v1 geometry and all eight action runs under null graphics, but slalom fails behavioral parity | **PASS for seven development contracts:** stable layout/obstacle IDs, exact manifest/configuration/artifact hashes, versioned behavior gates, evaluator-only schedule truth, and delivered BT/recovery/trajectory evidence; physical cause and controller consumption are not inferred | **PARTIAL overall:** seven layouts calibrated; slalom topology correction, repetition, and explanation evaluation remain |
