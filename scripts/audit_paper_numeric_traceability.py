...
def main() -> int:
    ...
    require_equal(len(physical_dispositions), 29, "physical cohort attempted configurations")
    require_equal(
        sum(
            item["admission"].get(
    ...
    require_equal(pointer(latest_physical, "/contract/fixed_order"), 29, "run 029 fixed order")
    require_equal(pointer(latest_physical, "/observed/navigation_status"), "aborted", "run 029 action status")
    ...
    require_equal(pointer(latest_physical, "/method_visible_reference/disposition"), "insufficient", "run 029 method-visible disposition")
    require_equal(pointer(latest_physical, "/method_visible_reference/command_samples"), 406, "run 029 command count")
    require_equal(pointer(latest_physical, "/method_visible_reference/odometry_samples"), 0, "run 029 masked odometry count")
    ...
    require_equal(pointer(latest_physical, "/progress/next_fixed_run_id"), "cm-land-conf-030", "next physical run")
    ...
    require_paper_fragments(
        [
            ...
            "twenty-nine valid,",
            "Physical cohort & 29/100 valid attempts",
        ]
    )
