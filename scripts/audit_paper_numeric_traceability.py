...
def main() -> int:
    ...
    require_equal(len(physical_dispositions), 30, "physical cohort attempted configurations")
    require_equal(
        sum(
            item["admission"].get(
    ...
    require_equal(pointer(latest_physical, "/contract/fixed_order"), 30, "run 030 fixed order")
    require_equal(pointer(latest_physical, "/observed/navigation_status"), "aborted", "run 030 action status")
    ...
    require_equal(pointer(latest_physical, "/diagnostic/independent_disposition"), "supported", "run 030 independent disposition")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/0"), 18.0, 0.0, "run 030 discrepancy start")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/1"), 39.0, 0.0, "run 030 discrepancy end")
    require_equal(pointer(latest_physical, "/progress/method_visible_diagnostic_supported"), 20, "supported diagnostic count")
    require_equal(pointer(latest_physical, "/progress/method_visible_diagnostic_insufficient"), 5, "insufficient diagnostic count")
    require_equal(pointer(latest_physical, "/progress/next_fixed_run_id"), "cm-land-conf-031", "next physical run")
    ...
    require_paper_fragments(
        [
            ...
            "thirty valid,",
            "Physical cohort & 30/100 valid attempts",
        ]
    )
