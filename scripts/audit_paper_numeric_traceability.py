...
def main() -> int:
    ...
    require_equal(len(physical_dispositions), 28, "physical cohort attempted configurations")
    require_equal(
        sum(
            item["admission"].get(
    ...
    require_equal(pointer(latest_physical, "/contract/fixed_order"), 28, "run 028 fixed order")
    require_equal(pointer(latest_physical, "/observed/navigation_status"), "aborted", "run 028 action status")
    ...
    require_equal(pointer(latest_physical, "/method_visible_reference/disposition"), "insufficient", "run 028 method-visible disposition")
    require_equal(pointer(latest_physical, "/method_visible_reference/command_samples"), 487, "run 028 command count")
    require_equal(pointer(latest_physical, "/method_visible_reference/odometry_samples"), 0, "run 028 masked odometry count")
    require_equal(pointer(latest_physical, "/progress/method_visible_diagnostic_supported"), 19, "supported diagnostic count")
    require_equal(pointer(latest_physical, "/progress/method_visible_diagnostic_insufficient"), 4, "insufficient diagnostic count")
    require_equal(pointer(latest_physical, "/progress/next_fixed_run_id"), "cm-land-conf-029", "next physical run")
    ...
    require_paper_fragments(
        [
            ...
            "twenty-eight valid,",
            "Physical cohort & 28/100 valid attempts",
        ]
    )
