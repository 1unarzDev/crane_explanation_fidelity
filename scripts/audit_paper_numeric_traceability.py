...
def main() -> int:
    ...
    require_equal(len(physical_dispositions), 26, "physical cohort attempted configurations")
    require_equal(
        sum(
            item["admission"].get(
    ...
    require_equal(pointer(latest_physical, "/contract/fixed_order"), 26, "run 026 fixed order")
    require_equal(pointer(latest_physical, "/observed/navigation_status"), "succeeded", "run 026 action status")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/0"), 18.0, 0.0, "run 026 discrepancy start")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/1"), 28.0, 0.0, "run 026 discrepancy end")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_commanded_planar_speed_mps"), 0.26, 0.0, "run 026 discrepancy command")
    ...
    require_close(pointer(latest_physical, "/diagnostic/response_recovery_interval_s/0"), 30.0, 0.0, "run 026 recovery start")
    require_close(pointer(latest_physical, "/diagnostic/response_recovery_interval_s/1"), 31.0, 0.0, "run 026 recovery end")
    require_equal(pointer(latest_physical, "/progress/transient_compensation"), 8, "transient-compensation count")
    require_equal(pointer(latest_physical, "/progress/method_visible_diagnostic_supported"), 18, "supported diagnostic count")
    require_equal(pointer(latest_physical, "/diagnostic/independent_disposition"), "supported", "run 026 diagnostic disposition")
    require_equal(pointer(latest_physical, "/progress/next_fixed_run_id"), "cm-land-conf-027", "next physical run")
    ...
    require_paper_fragments(
        [
            ...
            "twenty-six valid,",
            "Physical cohort & 26/100 valid attempts",
        ]
    )
