...
def main() -> int:
    ...
    require_equal(len(physical_dispositions), 10, "physical cohort attempted configurations")
    require_equal(
        sum(
            item["admission"].get(
    ...
    require_equal(pointer(latest_physical, "/contract/fixed_order"), 10, "run 010 fixed order")
    require_equal(pointer(latest_physical, "/observed/navigation_status"), "aborted", "run 010 action status")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/0"), 18.0, 0.0, "run 010 discrepancy start")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/1"), 39.0, 0.0, "run 010 discrepancy end")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_commanded_planar_speed_mps"), 0.26, 0.0, "run 010 command speed")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_measured_planar_speed_mps"), 0.0, 0.0, "run 010 measured speed")
    require_equal(pointer(latest_physical, "/progress/persistent_discrepancy"), 2, "persistent-discrepancy count")
    require_equal(pointer(latest_physical, "/progress/method_visible_diagnostic_supported"), 5, "supported diagnostic count")
    require_equal(pointer(latest_physical, "/progress/next_fixed_run_id"), "cm-land-conf-011", "next physical run")
    ...
    require_paper_fragments(
        [
            ...
            "ten valid,",
            "Physical cohort & 10/100 valid attempts",
        ]
    )
