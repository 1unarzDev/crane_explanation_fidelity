...
def main() -> int:
    ...
    require_equal(len(physical_dispositions), 13, "physical cohort attempted configurations")
    require_equal(
        sum(
            item["admission"].get(
    ...
    require_equal(pointer(latest_physical, "/contract/fixed_order"), 13, "run 013 fixed order")
    require_equal(pointer(latest_physical, "/observed/navigation_status"), "succeeded", "run 013 action status")
    ...
    require_equal(pointer(latest_physical, "/diagnostic/independent_disposition"), "not_triggered", "run 013 diagnostic disposition")
    require_equal(pointer(latest_physical, "/progress/next_fixed_run_id"), "cm-land-conf-014", "next physical run")
    ...
    require_paper_fragments(
        [
            ...
            "thirteen valid,",
            "Physical cohort & 13/100 valid attempts",
        ]
    )
