...
def main() -> int:
    ...
    require_equal(len(physical_dispositions), 11, "physical cohort attempted configurations")
    require_equal(
        sum(
            item["admission"].get(
    ...
    require_equal(pointer(latest_physical, "/contract/fixed_order"), 11, "run 011 fixed order")
    require_equal(pointer(latest_physical, "/observed/navigation_status"), "succeeded", "run 011 action status")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/0"), 10.0, 0.0, "run 011 discrepancy start")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_interval_s/1"), 20.0, 0.0, "run 011 discrepancy end")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_commanded_planar_speed_mps"), 0.26, 0.0, "run 011 command speed")
    require_close(pointer(latest_physical, "/diagnostic/discrepancy_measured_planar_speed_mps"), 0.0, 0.0, "run 011 measured speed")
    ...
    require_equal(pointer(latest_physical, "/progress/next_fixed_run_id"), "cm-land-conf-012", "next physical run")
    ...
    require_paper_fragments(
        [
            ...
            "eleven valid,",
            "Physical cohort & 11/100 valid attempts",
        ]
    )
