#!/usr/bin/env python3
"""Create a checked command-motion export with measured-motion samples withheld."""
...
MASK_ID = "missing-independent-odometry-samples-v1"
...
def build_masked_export(source: dict[str, Any], episode_id: str) -> dict[str, Any]:
    ...
    if checked["diagnostic_result"]["disposition"] != "insufficient":
        raise RuntimeError("missing-odometry mask did not produce an insufficient diagnosis")

    boundary = copy.deepcopy(method_payload["evidence_boundary"])
    limitations = list(boundary.get("limitations") or ())
    limitations.append(
        "Independently delivered odometry samples are unavailable in this evidence variant, so "
        "a time-aligned command-to-motion comparison cannot be computed."
    )
...
def main() -> int:
    ...
    result = build_masked_export(source, args.episode_id)
