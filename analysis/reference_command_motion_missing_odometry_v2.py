#!/usr/bin/env python3
"""Independently reference one missing-odometry ambiguity cluster."""
...
def build_reference(export: dict[str, Any], source_path: Path) -> dict[str, Any]:
    ...
    if mask.get("mask_id") != "remove-delivered-odometry-v1":
        raise ValueError("unexpected ambiguity mask")
    if mask.get("independent_scenario_increment") != 1:
        raise ValueError("ambiguity cluster is not independently configured")
    ...
    return {
        "schema": "crane-command-motion-missing-odometry-reference/v2",
        ...
        "statistical_cluster_id": episode_id,
...
def main() -> int:
    ...
    result = build_reference(
        json.loads(args.input.read_text(encoding="utf-8")), args.input
    )
