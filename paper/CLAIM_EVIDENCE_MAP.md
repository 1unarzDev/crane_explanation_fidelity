# Manuscript claim-to-evidence map

Status: living pre-manuscript audit, 2026-09-22. Every numerical claim in the eventual anonymous
paper must point to a retained manifest/result; this file is not itself experimental evidence.

| Claim candidate | Retained support | Current verdict |
| --- | --- | --- |
| Legacy provenance/checking improves response trustworthiness | 33 paired F/G/H episodes and 198 calls | **UNRESOLVED:** no sealed labels/adjudication; under frozen minimum |
| Legacy method provides exact runtime-to-source software-mechanism links | BT/source hashes, parity audits, checked plans across 33 episodes | **SUPPORTED AS IMPLEMENTATION/CAPTURE**, not as an error-rate improvement |
| Legacy method diagnoses physical causes | Legacy questions deliberately lack decisive physical evidence | **CONTRADICTED / DO NOT CLAIM** |
| Proposed diagnostic pipeline improves supported diagnostic success | Checked RoboBoat and land development outputs; no R/P/T/N comparison or independent labels | **UNRESOLVED / PROSPECTIVE STUDY NOT_RUN** |
| Diagnostic pipeline can express a bounded terminal-margin mechanism | Governed RoboBoat development export links exact Nav2 thresholds, measured return motion, post-result displacement, and settled error | **IMPLEMENTED/TESTED ON ONE DEVELOPMENT RUN**, not an effectiveness claim |
| Diagnostic pipeline can express a bounded geometric/execution mechanism | Governed current-source land export links a hash-checked global costmap, direct-route audit, delivered trajectory, exact 70 s BT source, action result, and final-text verification | **IMPLEMENTED/TESTED ON ONE DEVELOPMENT RUN**; supports direct-route restriction and deadline alignment, not global no-path |
| Proposed diagnostic method outperforms a tool-enabled repository agent | One parity-corrected, unannotated land development question; R received the exact diagnostic wrapper and produced the same central mechanism/limits as P/T | **NOT SUPPORTED IN DEVELOPMENT / CONFIRMATORY STUDY NOT_RUN**; no human labels or uncertainty estimate |
| LLM realization improves the checked diagnostic answer | One land candidate was substantively faithful but rejected by exact verification, making final P byte-identical to T | **UNRESOLVED**; development fallback 1/1 and no independent usefulness label |
| Ecological land behavior includes real path change/recovery/blockage | Three governed ecological exports plus a current-source valid terminal pilot. The current pilot records a lethal direct-route restriction, 2.705 m lateral detour, 69 successful planning updates, and deadline abort; its retained grid remains connected | **SUPPORTED FOR SIMULATION DEVELOPMENT**; prospective explanation comparison and independent labels `NOT_RUN` |
| RoboBoat has credible baseline navigation/docking | command-response tests, long path tracking, independent docking predicates, 8/8 post-correction docks | **SUPPORTED FOR DEVELOPMENT BASELINE**, prospective diagnosis `NOT_RUN` |
| Wave drift is diagnosed | no validated wave-force attribution or robot-visible discriminator | **DO NOT CLAIM** |
| Human trust or real-world reliability improved | no user study or hardware validation | **DO NOT CLAIM** |

The paper structure should be: (1) honestly bounded legacy provenance result, (2) diagnosis-to-
language method, (3) prospective land and focused surface-vehicle results if completed, and
(4) limitations distinguishing simulation relevance, factual correctness, diagnostic usefulness,
and human trust.
