...
| Land command--motion physical/reference cohort progress and bounded findings | `manifests/data/cm-land-conf-001-disposition.json` through `manifests/data/cm-land-conf-009-disposition.json` | Nine of 100 fixed confirmation configurations attempted and valid; runs 003 and 006 are mechanism-insufficient after independently configured odometry masks while retaining execution facts; runs 004 and 008 record recovered measured response followed respectively by abort and success; runs 005 and 007 are nominal `not_triggered` controls; run 009 is a persistent discrepancy followed by abort; no semantic calls or effect estimate |
