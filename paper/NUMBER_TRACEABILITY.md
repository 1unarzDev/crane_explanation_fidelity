...
| Land command--motion physical/reference cohort progress and bounded findings | `manifests/data/cm-land-conf-001-disposition.json` through `manifests/data/cm-land-conf-004-disposition.json` | Four of 100 fixed confirmation configurations attempted and valid; run 003 is mechanism-insufficient after the predeclared odometry mask while retaining execution facts; run 004 records recovered measured response followed by action abort; no semantic calls or effect estimate |
