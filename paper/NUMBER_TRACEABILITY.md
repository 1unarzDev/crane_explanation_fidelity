...
| Land command--motion physical/reference cohort progress and bounded findings | `manifests/data/cm-land-conf-001-disposition.json` through `manifests/data/cm-land-conf-015-disposition.json` | Fifteen of 100 fixed confirmation configurations attempted and valid; runs 003 and 006 are mechanism-insufficient after independently configured odometry masks while retaining execution facts; runs 004, 008, 011, and 015 record recovered measured response followed respectively by abort, success, success, and success; runs 005, 007, 012, and 013 are nominal `not_triggered` controls; runs 009, 010, and 014 are persistent discrepancies followed by abort; no semantic calls or effect estimate |
