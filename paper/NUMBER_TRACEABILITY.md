...
| Land command--motion physical/reference cohort progress and bounded findings | `manifests/data/cm-land-conf-001-disposition.json` through `manifests/data/cm-land-conf-006-disposition.json` | Six of 100 fixed confirmation configurations attempted and valid; runs 003 and 006 are mechanism-insufficient after independently configured odometry masks while retaining execution facts; run 004 records recovered measured response followed by action abort; run 005 is a nominal `not_triggered` control; no semantic calls or effect estimate |
